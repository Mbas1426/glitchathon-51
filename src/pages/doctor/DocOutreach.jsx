				...
				return (
					<div key={p.patient_id} style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderLeft: `3px solid ${rc}`, borderRadius: 14, padding: 16, transition: "border-color 0.2s" }}>
						<div
							...
							onClick={() => setExpanded(isExp ? null : p.patient_id)}
						>
							<div style={{ width: 38, height: 38, borderRadius: 10, fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", background: `${rc}18`, color: rc }}>
							...
							<div style={{ flex: 1 }}>
								...
								<div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>{diagIcon(p.diagnosis)} {p.diagnosis} · {p.last_test}:
									<span style={{ color: C.orange }}>{p.last_value}</span>
									· Overdue:
									<span style={{ color: C.red }}>{p.overdue_days}d</span>
								</div>
							</div>
							<div style={{ display: "flex", alignItems: "center", gap: 8 }}>
								{resp &&
									<span
										style={{
											...
											background: resp.type === "agreed" ? C.greenDim : resp.type === "declined" ? C.redDim : C.blueDim,
											color: resp.type === "agreed" ? C.green : resp.type === "declined" ? C.red : C.blue
										}}
									>
										{resp.type === "agreed" ? "✓ Agreed" : resp.type === "declined" ? "✗ Declined" : "? Questioned"}
									</span>
								}
							...
							<div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
								...
								{resp && (
									<div style={{ background: C.blueFaint, border: `1px solid ${C.blue}30`, borderRadius: 10, padding: 12, marginTop: 10 }}>
										...
										{resp.type === "agreed" &&
											<div style={{ marginTop: 8, fontSize: 11, color: C.green }}>✓ Home collection booked</div>
										}
										{resp.type === "declined" &&
											<div style={{ marginTop: 8, fontSize: 11, color: C.orange }}>↻ Re-outreach in 30 days</div>
										}
										{resp.type === "question" &&
											<div style={{ marginTop: 8, fontSize: 11, color: C.blue }}>◈ Contextual reply sent</div>
										}
									</div>
								)}
								<div style={{ display: "flex", gap: 8, marginTop: 12 }}>
									...
									{!p.has_smartphone &&
										<button style={{ background: C.orangeDim, border: `1px solid ${C.orange}50`, color: C.orange, padding: "8px 14px", borderRadius: 9, cursor: "pointer", fontSize: 11 }}>
											👨‍👩‍👧 Notify NOK
										</button>
									}
								</div>
